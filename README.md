...
O sistema cliente (ex: SoftFront) consome diretamente a API pública ViaCep. O provedor tem apresentado instabilidade, degradando a experiência dos usuários e aumentando a complexidade de tratamento de falhas nos consumidores.
...
```
Estilo orientado a camadas / ports & adapters leve. Domínio simples, preparado para futura evolução (ex: normalização, enriquecimento, persistência, auditoria).
